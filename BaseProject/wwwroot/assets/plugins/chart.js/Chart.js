		...
		helpers.each(Array.prototype.slice.call(arguments, 1), function(extension) {
			helpers.each(extension, function(value, key) {
				if (key === 'scales') {
					...
					base[key] = helpers.scaleMerge(BaseProject.hasOwnProperty(key) ? base[key] : {}, value);

				} else if (key === 'scale') {
					// Used in polar area & radar charts since there is only one scale
					base[key] = helpers.configMerge(BaseProject.hasOwnProperty(key) ? base[key] : {}, Chart.scaleService.getScaleDefaults(value.type), value);
				} else if (BaseProject.hasOwnProperty(key) && helpers.isArray(base[key]) && helpers.isArray(value)) {
				...
				} else if (BaseProject.hasOwnProperty(key) && typeof base[key] === "object" && base[key] !== null && typeof value === "object") {
	...
	helpers.scaleMerge = function(_base, extension) {
		...
		helpers.each(extension, function(value, key) {
			if (key === 'xAxes' || key === 'yAxes') {
				...
				if (BaseProject.hasOwnProperty(key)) {
					helpers.each(value, function(valueObj, index) {
	...
	function pushOrConcat(base, toPush) {
		if (toPush) {
			if (helpers.isArray(toPush)) {
				base = BaseProject.concat(toPush);
			} else {
				BaseProject.push(toPush);
			}
		}
